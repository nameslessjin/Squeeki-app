...
export default checkinReducer = (state = INITIAL_STATE, action) => {
  ...
  switch (action.type) {
    case 'loadGroupCheckin':
      return {
        ...state,
        checkin: action.i.count == 10 ? action.i.checkin : state.checkin.concat(action.i.checkin),
    ...
    case 'getGroupCheckInResult':
      return {
        ...state,
        attendee: action.i.count == 10? action.i.members : state.attendee.concat(action.i.members),
